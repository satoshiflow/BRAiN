...
**Aktualisiert:** 2026-02-18 08:15 Uhr  
**Status:** Tasks 3.2-3.4 abgeschlossen, nächste Schritte definiert

---

## 🚨 KRITISCH: Vor allen anderen Tasks

### 0.1 Coolify Build Cache löschen
**Problem:** Backend startet nicht trotz Fix (alter Build im Cache)
**Priorität:** 0.1 (ABSOLUTE TOP-PRIORITÄT)

**Lösung:**
- [ ] Coolify → brain-backend → Configuration
...
# Sollte: {"status":"ok"...} zurückgeben (nicht 502)
```

**Zeit:** ~10 Minuten  
**Warum kritisch:** Ohne das läuft GAR NICHTS. Backend = 502 Bad Gateway.

---

### 0.2 Uncommitted Files commiten
**Priorität:** 0.2 (Repo-Sauberkeit)
...
**Zeit:** ~5 Minuten  
**Warum wichtig:** Sauberes Repo, keine uncommitted Änderungen vor dem Deploy.

---

## 🔥 PRIORITÄT 1: Production Deployment & Testing
...
- [ ] Hierarchie wird korrekt zurückgegeben
...
## 🔴 PRIORITÄT 2: Genesis Integration (Agent Spawning)
...
## 🟠 PRIORITÄT 3: Auto-Scaling Logic
...
- [ ] Alle aktiven Cluster werden geprüft
...
## 🟡 PRIORITÄT 4: Metrics & Monitoring
...
## 🟢 PRIORITÄT 5: Chat Endpoint Fix (CRITICAL)

### 5.1 Chat Endpoint 404 Problem beheben + AXE UI Browser Test
**Problem:** `/api/chat` zeigt 404 trotz Commit  
**Priorität:** KRITISCH für AXE UI
...
**Akzeptanzkriterien:**
- [ ] `/api/chat/health` gibt 200 zurück
- [ ] `/api/chat` funktioniert
- [ ] AXE UI kann chatten

**AXE UI Browser Test (NICHT nur curl!):**
- [ ] https://axe.brain.falklabs.de öffnen (echter Browser!)
- [ ] Login testen
- [ ] Chat-Nachricht senden
- [ ] Antwort vom LLM erhalten
- [ ] UI-Elemente prüfen (Buttons, Layout, Responsiveness)
- [ ] Console auf JavaScript-Fehler checken

**Zeit:** ~45 Minuten (inkl. Browser-Test)

---

## 📋 TAG 1 CHECKLISTE (2026-02-19)

### Morgens erstes (45 Min):
- [ ] 0.1 Coolify Cache löschen & Rebuild
- [ ] 0.2 Uncommitted Files commiten
- [ ] 1.1 Health Check: Backend läuft
- [ ] 1.2 Cluster API testen
- [ ] 5.1 Chat Endpoint fixen & AXE UI Browser-Test

**✅ BLOCKER-RESOLUTION PHASE (Muss funktionieren bevor wir weiter machen!)**
...
- [ ] Nächste TODO für Tag 2 erstellen

---

## 🧠 DEBUGGING TIPS & ERINNERUNGEN
...
4. **Nach Push: Coolify Cache löschen wenn Probleme**

### Debugging:
```bash
# Coolify Logs live ansehen
curl -s "http://46.224.37.114:8000/api/v1/applications" \
  -H "Authorization: Bearer juq0nGzKcKvzOhjlfXkNct6GZmVhE9em6wrmwYXu8bb8d0e7"
...
|--------|---------|--------|
| 502 Bad Gateway | Backend startet nicht | Coolify Cache löschen |
| 404 auf Endpoint | Router nicht registriert | main.py checken |
| Limiter Error | `@limiter.limit` ohne request | Decorator fixen |
| Import Error | Module nicht gefunden | `__init__.py` checken |

### Backup / Safety:
- [ ] Datenbank-Backup vor großen Änderungen
- [ ] Migrationen immer testen vor Deploy
- [ ] Feature-Branches für große Changes nutzen

### Kommunikation:
- **Probleme?** → Sofort logs schicken
- **Erfolge?** → Kurze Status-Updates
- **Blocker?** → Nicht lange warten, fragen!
...
| Health Check | https://api.brain.falklabs.de/health |
| API Docs | https://api.brain.falklabs.de/docs |
...
**Gesamt-Schätzung:** ~11 Stunden

### Tag 1 (Heute - 2026-02-19):
| Task | Zeit | Status |
|------|------|--------|
| 0.1 Coolify Cache löschen | 10 min | 🔴 OFFEN |
| 0.2 Uncommitted Files | 5 min | 🔴 OFFEN |
| 1.1 Backend Deployment | 10 min | 🔴 OFFEN |
| 1.2 Cluster API Test | 20 min | 🔴 OFFEN |
| 5.1 Chat Fix + Browser Test | 45 min | 🔴 OFFEN |
| **BLOCKER RESOLUTION** | **~1.5h** | **KRITISCH** |
| 2.1 Genesis Analyse | 30 min | 🔴 OFFEN |
| 2.2 Genesis Integration | 90 min | 🔴 OFFEN |
| 3.1 Auto-Scaling Logic | 90 min | 🔴 OFFEN |
| **Total Tag 1** | **~5.5h** | |

### Tag 2 (2026-02-20):
- ✅ Priorität 3.2: Auto-Scaling Background (1h)
- ✅ Priorität 4: Metrics Collection (2h)
- ✅ Optional: Manifests Generator (1h)
- **Total:** ~4 Stunden

---

## ✅ BEREITS ERLEDIGT (2026-02-18)

- ✅ Task 3.1: Database Migration (Max)
- ✅ Task 3.2: Blueprint Loader & Validator
- ✅ Task 3.3: Cluster Service (create, scale, hibernate, hierarchy)
- ✅ Task 3.4: API Endpoints (alle 14 Endpoints)
- ✅ Spawner Grundstruktur
- ✅ Test Suite (10/10 Tests)
- ✅ Blueprints Router registriert
- ✅ Committed & gepusht
...
- ✅ Cluster aus Blueprint erstellen
- ✅ Agents spawnen (mit Genesis Integration)
- ✅ Scaling funktioniert (manuell)
- ✅ Hierarchie abrufen
...
## 🚀 SUBAGENT-AUFGABEN (Für Claude)

### Claude Task 1: Genesis Integration
**Komplexität:** Hoch  
**Sicherheitsrelevant:** Ja (Agent-Erstellung)  
**Prompt:** Siehe `/tmp/claude_task1_genesis.md`

### Claude Task 2: Auto-Scaling Logic
**Komplexität:** Mittel  
**Sicherheitsrelevant:** Nein  
**Prompt:** Siehe `/tmp/claude_task2_autoscale.md`

### Claude Task 3: AXE UI Browser Test
**Komplexität:** Niedrig  
**Sicherheitsrelevant:** Nein  
**Prompt:** Siehe `/tmp/claude_task3_axe_test.md`
...
- Genesis Integration unklar? → Genesis Modul Docs lesen
- API Tests fehlschlagen? → Backend Logs checken
- Performance Issues? → Metrics analysieren

**Nächster Review:** Nach Task 0.1 + 0.2 (Backend läuft)

---

**Status:** 📋 **READY TO START**  
**Nächster Task:** 0.1 Coolify Cache löschen  
**Blocker:** Backend 502 → Cache löschen erforderlich
