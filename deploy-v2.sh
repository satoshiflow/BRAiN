...
#############################################
# Step 1: Backup old /opt/brain/ installation
#############################################
log_info "Step 1/6: Backing up old /opt/brain/ installation..."
...
if [ -d "/opt/brain" ]; then
    ...
    # Rename to brain_old
    log_info "Backing up /opt/brain to /opt/brain_old..."
    cd /opt
    if [ -d "/opt/brain_old" ]; then
        log_warning "Removing existing /opt/brain_old..."
        rm -rf brain_old
    fi
    mv brain brain_old
    log_success "Backup created at /opt/brain_old"
else
    log_info "/opt/brain/ directory not found - fresh installation"
fi

#############################################
# Step 2: Clone V2 Repository
#############################################
log_info "Step 2/6: Cloning V2 repository to /opt/brain..."

cd /opt
git clone -b claude/migrate-v2-launch-01UQ1FuiVg8Rv6UQwwDar1g5 https://github.com/satoshiflow/BRAiN.git brain
...
#############################################
CORS_ORIGINS=["http://localhost:8000","http://localhost:8000","http://localhost:3000","http://localhost:3001","http://localhost:8080","https://brain.falklabs.de","https://dev.brain.falklabs.de"]
...
#############################################
log_info "Step 6/6: Checking Ollama data migration..."
...
if [ "$REUSE_OLLAMA" = true ] && [ -n "$OLD_OLLAMA_VOLUME" ]; then
    ...
    docker restart brain-ollama
    sleep 5
else
    log_info "No existing Ollama data to migrate"
    log_warning "You will need to download Ollama models manually:"
    log_warning "  docker exec brain-ollama ollama pull phi3"
    log_warning "  docker exec brain-ollama ollama pull llama3.2"
fi

#############################################
# Step 7: Health checks
#############################################
log_info "obsolete: Running health checks..."
...
if curl -f http://localhost:3000 > /dev/null 2>&1; then
    log_success "✓ Control Deck is running (http://localhost:3000)"
else
...
if curl -f http://localhost:3001 > /dev/null 2>&1; then
    log_success "✓ AXE UI is running (http://localhost:3001)"
else
...
if curl -f http://localhost:6333 > /dev/null 2>&1; then
    log_success "✓ Qdrant is running (http://localhost:6333)"
else
...
if docker exec brain-postgres pg_isready -U brain > /dev/null 2>&1; then
...
if docker exec brain-redis redis-cli ping > /dev/null 2>&1; then
...
log_info "  • Control Deck:   http://localhost:3000"
log_info "  • AXE UI:         http://localhost:3001"
...
if [ "$REUSE_OLLAMA" != true ]; then
    ...
    log_info "  docker exec brain-ollama ollama pull phi3"
    log_info "  docker exec brain-ollama ollama pull llama3.2"
    echo ""
else
    log_success "Ollama models were migrated from old installation! ✓"
    log_info "Check available models:"
    log_info "  docker exec brain-ollama ollama list"
    echo ""
fi
