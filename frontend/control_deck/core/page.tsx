...
const DEFAULT_AGENT_ID = "agent_test_1";

export default function CoreOverviewPage() {
  const [agentId, setAgentId] = useState(DEFAULT_AGENT_ID);

  const [modulesState, setModulesState] = useState<LoadState<UIModuleManifest[]>>({
    loading: true,
  });
  ...
  const missionsModule = useMemo(
    () =>
      modulesState.data?.find((m) => m.name === "missions") ??
      modulesState.data?.[0],
    [modulesState.data],
  );
    ...
    <div className="flex flex-col gap-6 p-6">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-neutral-400">
          Missions · DNA · KARMA · Immune – Übersicht für den aktuellen Agenten.
        </p>
      </header>
      ...
      <section className="flex flex-wrap items-center gap-3">
        <div>
          ...
          <input
            className="rounded-md border border-neutral-700 bg-neutral-900 px-3 py-1.5 text-sm text-white focus:outline-none focus:ring-1 focus:ring-sky-500"
            value={agentId}
            onChange={(e) => setAgentId(e.target.value)}
            placeholder="agent_test_1"
          />
        </div>
        {dnaState.error && (
          <span className="text-xs text-red-400">
            DNA konnte nicht geladen werden: {dnaState.error}
          </span>
        )}
      </section>

      {/* Grid: Missions / DNA+KARMA / Immune */}
