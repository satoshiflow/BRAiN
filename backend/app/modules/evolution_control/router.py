...
from .schemas import EvolutionProposalCreateResponse, EvolutionProposalResponse, EvolutionProposalTransitionRequest
from .service import get_evolution_control_service


router = APIRouter(prefix="/api/evolution", tags=["evolution-control"], dependencies=[Depends(require_auth)])
...
async def get_evolution_proposal(
    ...
    proposal = await get_evolution_control_service().get_by_id(db, proposal_id, tenant_id)
    if proposal is None:
...
@router.post("/proposals/patterns/{pattern_id}", response_model=EvolutionProposalCreateResponse, status_code=status.HTTP_201_CREATED)
async def create_evolution_proposal(
    ...
    try:
        proposal = await get_evolution_control_service().create_from_pattern(db, pattern_id, principal)
    except ValueError as exc:
        message = str(exc)
        if message == "Tenant context required":
            raise HTTPException(status_code=403, detail=message) from exc
        raise HTTPException(status_code=404, detail=message) from exc
    return EvolutionProposalCreateResponse(pattern_id=pattern_id, proposal=EvolutionProposalResponse.model_validate(proposal))


@router.post("/proposals/{proposal_id}/transition", response_model=EvolutionProposalResponse)
async def transition_evolution_proposal(
    ...
    await _ensure_evolution_control_writable(db)
    try:
        proposal = await get_evolution_control_service().transition_status(db, proposal_id, principal, payload.status)
    except ValueError as exc:
        message = str(exc)
        if message == "Tenant context required":
            raise HTTPException(status_code=403, detail=message) from exc
