...
def test_evolution_create_get_transition(monkeypatch) -> None:
    ...
    class FakeService:
        ...
        async def transition_status(self, db, proposal_id_param, principal, status, metadata_patch):
            assert proposal_id_param == proposal_id
            assert status == "review"
            assert metadata_patch == {}
    ...
    transition_response = client.post(
        ...
        json={"status": "review", "metadata_patch": {}},
    )
...
def test_evolution_create_from_skill_run(monkeypatch) -> None:
    app = FastAPI()
    app.include_router(evolution_router)
    ...
    proposal = SimpleNamespace(
        ...
        proposal_metadata={"proposal_mode": "proposal_only"},
        created_at=now,
        updated_at=now,
    )

    class FakeService:
        async def create_from_skill_run(self, db, skill_run_id_param, principal):
            assert skill_run_id_param == skill_run_id
    ...
    response = client.post(f"/api/evolution/proposals/skill-runs/{skill_run_id}")
    assert response.status_code == 201
    payload = response.json()
    assert payload["skill_run_id"] == str(skill_run_id)
    assert payload["proposal"]["id"] == str(proposal_id)
...
def test_evolution_transition_invalid_returns_400(monkeypatch) -> None:
    ...
    class FakeService:
        async def transition_status(self, db, proposal_id, principal, status, metadata_patch):
    ...
    response = client.post(
        ...
        json={"status": "applied", "metadata_patch": {}},
    )
    assert response.status_code == 400


def test_evolution_adaptive_freeze_endpoints(monkeypatch) -> None:
    app = FastAPI()
    app.include_router(evolution_router)
    ...
    class FakeService:
        async def get_freeze_status(self, db, tenant_id):
            assert tenant_id == "tenant-a"
            return unfrozen

        async def set_adaptive_freeze(self, db, tenant_id, principal, enabled, reason):
            assert tenant_id == "tenant-a"
            return frozen if enabled else unfrozen

    monkeypatch.setattr(
        route_module, "get_evolution_control_service", lambda: FakeService()
    )

    status_resp = client.get("/api/evolution/adaptive-freeze/status")
    assert status_resp.status_code == 200
    assert status_resp.json()["adaptive_frozen"] is False

    enable_resp = client.post("/api/evolution/adaptive-freeze/enable", json={"reason": "maintenance"})
    assert enable_resp.status_code == 200
    assert enable_resp.json()["adaptive_frozen"] is True

    disable_resp = client.post("/api/evolution/adaptive-freeze/disable", json={"reason": "resume"})
    assert disable_resp.status_code == 200
    assert disable_resp.json()["adaptive_frozen"] is False


def test_evolution_ops_summary(monkeypatch) -> None:
    app = FastAPI()
    app.include_router(evolution_router)
    ...
    class FakeService:
        async def get_ops_summary(self, db, tenant_id):
            assert tenant_id == "tenant-a"
            return {
                "tenant_id": tenant_id,
                "adaptive_frozen": False,
                "proposal_counts": {"review": 2, "rejected": 1},
    ...
    response = client.get("/api/evolution/ops/summary")
